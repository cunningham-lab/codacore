...
trainRelaxation.CoDaBaseLearner = function(cdbl) {
  ...
  if (cdbl$mode == "classification") {
    if (abs(mean(1 / (1 + exp(-cdbl$boostingOffset))) - mean(cdbl$y)) < 0.001) {
      ...
      tempGLM = glm(cdbl$y ~ 1, offset=cdbl$boostingOffset, family='binomial')
  ...
  if (cdbl$type == 'A') {
    ...
    forwardPass = function(x, mask = NULL) {
      ...
      logRatio = k_dot(keras::k_log(x), pvePart) / keras::k_maximum(keras::k_sum(pvePart), epsilon) -
        k_dot(keras::k_log(x), nvePart) / keras::k_maximum(keras::k_sum(nvePart), epsilon)
      eta = self$slope * logRatio + self$intercept + self$boostingOffset
      keras::k_sigmoid(eta)
    }
  }
...
findBestCutoff.CoDaBaseLearner = function(cdbl) {
  ...
  if (cdbl$verbose) {
    print('CV time:')
    print(endTime - startTime)
    plot(2:maxCutoffs, means, ylim=range(c(means-stds, means+stds)))
    arrows(2:maxCutoffs, means-stds, 2:maxCutoffs, means+stds, length=0.05, angle=90, code=3)
    abline(gammaSeRule, 0)
  }
...
setInterceptAndSlope.CoDaBaseLearner = function(cdbl, x, y, boostingOffset) {
  ...
  if (cdbl$mode == "classification") {
    glm = glm(y~x, family='binomial', data=dat, offset=boostingOffset)
  } else if (cdbl$mode == "regression") {
    warning("Not tested")
    glm = glm(y~x, family='normal', data=dat, offset=boostingOffset)
  } else {
...
plot.codacore = function(cdb) {
  ...
  for (i in 2:min(4, length(cdb$ensemble))) {
    lines(cdb$ensemble[[i]]$ROC$specificities, cdb$ensemble[[i]]$ROC$sensitivities, col=cols[i], lwd=lwds[i])
  }
  legend("bottomright",
